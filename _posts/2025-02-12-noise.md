...
$$
\begin{aligned}
\hat{\boldsymbol{w}} = \argmin_{\boldsymbol{w}} \ \frac{1}{n}\sum_{i=1}^n (y_i - \boldsymbol{w}^\top \boldsymbol{x}_i)^2.
\end{aligned}
$$
{% endkatexmm %}
...
\begin{aligned}
\hat{\boldsymbol{w}} 
&= \argmin_{\boldsymbol{w}} \ \mathbb{E}_{\boldsymbol{\delta} \sim \mathcal{N}(\boldsymbol{0}, \boldsymbol{\Sigma})} \left[ \frac{1}{n} \sum_{i=1}^n (y_i - \boldsymbol{w}^\top \tilde{\boldsymbol{x}}_i)^2 \right], \\
...
\begin{aligned}
\hat{\boldsymbol{w}} 
&= \argmin_{\boldsymbol{w}} \ \frac{1}{n} \sum_{i=1}^n \left((y_i - \boldsymbol{w}^\top \boldsymbol{x}_i)^2 - 2 (y_i - \boldsymbol{w}^\top \boldsymbol{x}_i) \boldsymbol{w}^\top \mathbb{E}_{\boldsymbol{\delta} \sim \mathcal{N}(\boldsymbol{0}, \boldsymbol{\Sigma})} \left[\boldsymbol{\delta}_i\right] + \boldsymbol{w}^\top \mathbb{E}_{\boldsymbol{\delta} \sim \mathcal{N}(\boldsymbol{0}, \boldsymbol{\Sigma})} \left[ \boldsymbol{\delta}_i \boldsymbol{\delta}_i^\top \right] \boldsymbol{w} \right), \\
&= \argmin_{\boldsymbol{w}} \ \frac{1}{n} \sum_{i=1}^n (y_i - \boldsymbol{w}^\top \boldsymbol{x}_i)^2 + \boldsymbol{w}^\top \boldsymbol{\Sigma} \boldsymbol{w},
\end{aligned}
$$
{% endkatexmm %}
